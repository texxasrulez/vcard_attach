...
class vcard_attach extends rcube_plugin
{
    ...
    const PLUGIN_INFO = array(
        ...
        'license' => 'GPL-3.0',
        'uri' => 'https://github.com/texxasrulez/vcard_attach',
    );
    ...
    function render_page($args)
    {
        ...
        } else if ($args['template'] == 'addressbook' && rcube_utils::get_input_value('_source', rcube_utils::INPUT_GPC) == 'collected') {
    ...
    function prefs_table($args)
    {
        if ($args['section'] == 'compose') {
            ...
            if ($attach_vcard) {
                ...
                $content = $select->show((int) $rcmail->config->get('attach_vcard_from', 0), array('class' => 'vcard-attach-source-select', 'name' => "_attach_vcard_from", 'onchange' => 'if(this.value == 1){alert(\'' . $this->gettext('vcard_attach.warning') . '\');} $(\'.mainaction\').hide(); document.forms.form.submit();'));
                $append = '';
                if ($rcmail->config->get('attach_vcard_from') == 1) {
                    ...
                    $ul = html::tag('ul', array('class' => 'vcard-attach-contact-list'), $li);
                    $append = html::tag('fieldset', array('id' => 'vcard_attach_contacts', 'class' => 'vcard-attach-contact-box'), html::tag('legend', array('class' => 'vcard-attach-contact-legend'), $this->gettext('editcontact')) . $ul);
                }
                $args['blocks']['main']['options']['attach_vcard_from'] = array('title' => '<!-- --><br />&raquo;&nbsp;' . html::label($field_id, rcube_utils::rep_specialchars_output($this->gettext('vcard_attach.from'))), 'content' => html::tag('table', array('cellpadding' => '0', 'cellspacing' => '0', 'class' => 'vcard-attach-source-table'), html::tag('tr', null, html::tag('td', array('class' => 'vcard-attach-source-select-cell'), $content) . html::tag('td', null, $append))), );
            }
        }
        return $args;
    }
        ...
        if (file_exists($file)) {
            ...
            foreach ($identities as $key => $identity) {
                ...
                if ($identity['identity_id'] == $iid) {
                    if ($rcmail->config->get('attach_vcard_from', 0) == 1) {
